...
skills_delimned <- job_skills_dirty |>
  distinct(skill_name) |>
  mutate(skill_id = row_number()) |>
  relocate(skill_id, .before = skill_name)

# update joiner to bring unique artist idea 
job_skills_delimned <- job_skills_dirty |>
  left_join(skills_delimned, join_by(skill_name == skill_name)) |>
  subset(select=-c(skill_name))
```

This one is the same as above but uses distint to create individual rows of one unique skill per row.

```{r create-joiner-table-distinct}
...
write_parquet(skills_delimned, "datasets/skills_delimned.parquet")
write_parquet(job_skills_delimned, "datasets/job_skills_delimned.parquet")
...
write.csv(skills_delimned, "datasets/skills_delimned.csv", row.names=FALSE)
write.csv(job_skills_delimned, "datasets/job_skills_delimned.csv", row.names=FALSE)
write.csv(skills_distinct, "datasets/skills.csv", row.names=FALSE)
write.csv(job_skills_distinct, "datasets/job_skills.csv", row.names=FALSE)
