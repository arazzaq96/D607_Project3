...
job_skills_import <- read_csv("indeed/job_skills.csv") |>
  ...
  rename(job_name = job_skills)
  


# create joiner table
job_skills_key <- job_skills_import |>
  separate_longer_delim(cols = c(job_name), delim="," )
...
job_skills_df <- job_skills_key |>
  ...
  relocate(skill_id, .before = job_name)

# join artist list to keys to bring unique artist idea 
job_skills_key <- job_skills_key |>
  left_join(job_skills_df, join_by(job_name == job_name)) |>
  subset(select=-c(job_name))
